    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const teamMemberResponse = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001/api"}/team-members`,
          {
            headers: {
        ...
        for (const barber of barberList) {
          const serviceResponse = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001/api"}/services/team-member/${barber.id}`,
            {
              headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
              }
        ...
        setError(err instanceof Error ? err.message : 'Failed to load barbers and services');
  ...
  return (
    <main className="flex flex-col gap-20 mt-40">
      {barbers.length === 0 ? (
        ...
        barbers.map((barber) => (
          <section key={barber.id} className="container mx-auto flex flex-col md:flex-row justify-center items-center py-20 gap-8 px-4">
            ...
            <div className="flex flex-col gap-4 w-full md:w-1/3">
              ...
              <p>[IG@{barber.first_name.toLowerCase()}.barber] ({barber.status})</p>
              ...
              {services[barber.id]?.length > 0 ? (
                  ...
                  {services[barber.id].map((service) => (
                    <AccordionItem key={service.id} value={`service-${service.id}`}>
                      <AccordionTrigger className="bg-[#3D3D3D] text-white rounded-xl px-6 py-4 w-full">
                        <b>{service.name}</b>
                        <br />
                        <sub>${(service.price_amount / 100).toFixed(2)} {service.price_currency} ・ {service.duration} min</sub>
                      </AccordionTrigger>
                      <AccordionContent className="px-10 py-4">
                        <p>{service.description || "Experience a professional cut tailored to your style."}</p>
                        <Button 
                ...
                <p className="text-center py-4">No services available for this barber.</p>
              )}
            </div>
          </section>
        ))
