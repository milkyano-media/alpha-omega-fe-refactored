...
export default function ServiceSelection() {
  ...
  const handleServiceToggle = (service: Service, checked: boolean) => {
    setSelectedServices(prev => {
      if (checked) {
        ...
        return prev.some(s => s.id === service.id) ? prev : [...prev, service];
      } else {
  ...
  const calculateTotalPrice = () => {
    return selectedServices.reduce((total, service) => total + service.price_amount, 0);
  };
  ...
  return (
    <main className="min-h-screen bg-white">
      <div className="mx-auto px-4 py-6 pt-20 pb-32">
        {/* Header Section */}
